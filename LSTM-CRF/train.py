...
import data_preprocess as loader

optparser = optparse.OptionParser()
optparser.add_option("--config", default="./config.yaml", type=str, help="config file path")
optparser.add_option("--tag_scheme", default="iob",help="Tagging scheme, IOB or IOBES")
...
config = AttrDict(yaml.load(open(opts.config, 'r'), Loader=yaml.FullLoader))
# to beifen
...
def train(model, train_data,test_data, dev_data, id_to_tag,tag_to_id ):
    ...
    best_dev_f1 = -1
    best_test_f1 = -1
    tqdm_train_iter = tqdm(Conll2003Dataloader(train_data, batch_size, shuffle=True))
    loss = 0.0
    #count = 0
    all_count=0
    model.train( )
    for epoch in range(1, epoch_num + 1):
        ...
        for i, data in enumerate(tqdm_train_iter):
            ...
            tqdm.set_description(tqdm_train_iter, "loss: %.4f" % loss)
...
def main():
    ...
    #prepare word_embeds
    all_word_embeds = {}
    with open(parameters['pre_emb'], 'r', encoding='utf-8') as f:
